...
-- ============================================================================
-- This script will:
-- 1. Drop the old "registration" table (singular) if it exists
-- 2. Create fresh "registrations" table (plural) with correct session IDs
-- 3. Create all other necessary tables
--
-- IMPORTANT: This will delete any existing data in the registration table.
-- ============================================================================

-- Drop old tables if they exist
DROP TABLE IF EXISTS registration CASCADE;
DROP TABLE IF EXISTS registrations CASCADE;
DROP TABLE IF EXISTS identity_alerts CASCADE;
...
-- Indexes
CREATE INDEX aa_user_idx ON assessment_attempts(user_id);
CREATE INDEX aa_identity_idx ON assessment_attempts(identity_key);
CREATE INDEX aa_name_idx ON assessment_attempts(normalized_name);
CREATE INDEX aa_started_desc_idx ON assessment_attempts(started_at DESC);

-- Constraint: Only one active attempt per user
CREATE UNIQUE INDEX aa_one_started_per_user
...
CREATE INDEX identity_alerts_key_idx ON identity_alerts(identity_key);
CREATE INDEX identity_alerts_created_idx ON identity_alerts(created_at DESC);
...
BEGIN
  ...
  RAISE NOTICE '✓ Registrations table has correct session IDs:';
  ...
  RAISE NOTICE '✓ Database is ready for registrations!';
END $$;

-- ============================================================================
-- SETUP COMPLETE!
-- ============================================================================
-- Your database has been reset and is ready for the registration system.
-- You should see success messages above.
--
-- Next: Test your registration form
-- ============================================================================
